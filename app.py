...
def send_email_notification(email, pin):
    ...
    try:
        ...
        print(f"Email sent to {email} with PIN {pin}")
...
def register_visitor():
    ...
    visitor = Visitor(name=name, phone=phone, email=email, host_name=host_name, purpose=purpose, pin=pin, expiration_time=expiration_time)

    try:
        db.session.add(visitor)
        db.session.commit()
        send_email_notification(email, pin)
        return jsonify({"message": "Visitor registered successfully.", "pin": pin}), 201
    except Exception as e:
        db.session.rollback()
...
def validate_pin():
    ...
    visitor = Visitor.query.filter_by(pin=pin).first()

    if not visitor:
        return jsonify({"error": "Invalid PIN."}), 404

    if visitor.is_used:
        return jsonify({"error": "PIN has already been used."}), 400
    ...
    return jsonify({"message": "Access granted.", "visitor": {
        "name": visitor.name,
